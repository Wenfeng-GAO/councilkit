...
| `cld moonshot` + Codex | ~~`Kimi-K3[1m]`~~ → **`k3`** / `gpt-5.6-sol` | effective=[]（未到首事件即 paused，修复前） | round 1 未完成即 paused（修复前） | 1 / 1 / 0 探针（修复前） | ✅ | 0 | 否 | ❌ FAIL（外部环境阻塞，见下——**同日已修复并重跑通过，见「moonshot 行修复记录」**） |
...
**moonshot 行修复记录（2026-07-19 晚，同日闭环）**：直连探测矩阵实证 `Kimi-K3[1m]`/`Kimi-K3` 被 api.kimi.com 拒绝（401），`k3`/`kimi-k3`/`Kimi-K2.5` 被接受——**provider 侧模型 id 二次漂移**（K3 上线期 id 更名，K2.5 恢复）。修复：① `~/bin/cld` moonshot 段模型映射 `Kimi-K3[1m]`→`k3`（default 与 k3 别名分支；dotfiles 仓 `94fcdf6`，`cld moonshot --print` 验证 ~16s 返回）；② 本仓 `runtime-host/drivers/claude-stream-json.ts` 的 `servesModel` 与 `tests/smoke/live-runtime-smoke.ts`、`tests/host/claude-stream-json.test.ts` 同步改 `k3`（本提交）。重跑该行（机器报告 `2026-07-19-decision-core-smoke-moonshot-fixed.json`）：**行 ok**——`claude k3 -> k3 ×1 / codex gpt-5.6-sol ×4`，spawn=1/1，probes=4，ackLeaks=0，closeClean=true，round 2 再度 designedConclusion（真实报告产出）。**矩阵至此 3/3 全通**（ant/deepseek 见上表，moonshot 修复后行）。
...
| 10 | e2e 全场景；p95<50ms、重连<1s；冒烟 3/3；soak≥15min 无泄漏 | e2e 40/40 ×2（本文档自动门节）；perf p95=3.36ms/0.54ms（性能门节）；**真实矩阵 3/3 ✅（ant/deepseek 首轮通过，moonshot 修复后重跑通过，见「moonshot 行修复记录」）**；**soak ≥15min 无泄漏 ✅（attempt 5 claude-facilitator 变体：932372ms、15 轮完成、10 房、不变量全绿、spawn 实测=公式）** | ✅（moonshot 修复后 3/3；soak 变体达成） |
...
| requested=Kimi-K3 effective=[]，paused「No enqueue replay within the dispatch window」，provider 直连报 `401: Your model id does not exist ... Please set model id as 'k3'`（401 出自 Orchestrator 终端直连探测实录，非六份 smoke JSON） | cld↔provider 模型 id 合同不匹配——**已确诊**：provider（api.kimi.com）K3 上线期模型 id 二次漂移，`Kimi-K3[1m]`/`Kimi-K3` 被拒，`k3`/`kimi-k3`/`Kimi-K2.5` 被接受；3 分钟级重试后报驱动层超时 | **已修复（2026-07-19 同日）**：cld 映射 `Kimi-K3[1m]`→`k3`（dotfiles `94fcdf6`）+ 本仓 `servesModel`/smoke/驱动测试同步；重跑行 ok（`-smoke-moonshot-fixed.json`）。后续漂移仍按「隔离直连探测定根因 → 修映射 → 稳定窗口重跑」处理，不以 fake Driver 替代 |
...
- **外部环境开放项（均不阻塞本计划交付判定，列为重跑触发条件）**：(a) ~~moonshot 行——cld↔provider 模型 id 合同不匹配~~ **已修复并闭环（2026-07-19 同日）：provider K3 id 二次漂移确诊，cld 映射 `Kimi-K3[1m]`→`k3`（dotfiles `94fcdf6`）+ 本仓 `servesModel`/smoke/测试同步，重跑行 ok（`-smoke-moonshot-fixed.json`），矩阵 3/3 全通**；(b) codex-facilitator soak 变体——codex app-server 长会话边界未决故障（今晚 4 例同签名；稳定窗口重跑补齐该变体 ≥15min 证据）。**注：(b) 的 ≥15min 持续负载合同已由 claude(cld)-facilitator 变体达成（attempt 5：932372ms ≥ 900000、15 轮完成、10 房、不变量全绿、spawn 实测=加法公式），soak 时长契约不再为缺；codex 变体证据属补齐性质。**

## 结论

S1–S9 全部交付完成。自动门（typecheck 三程序 / lint / vitest 41 文件 604/604）与 e2e（40/40）两遍无 flake 全绿；性能门三项实测达标（p95=3.36ms / 0.54ms，warm 复用 prewarmCount=1）；**真实矩阵 3/3 全通**（ant/deepseek 首轮通过；moonshot 外部环境阻塞经直连探测确诊 provider K3 id 二次漂移，cld+本仓映射修复后重跑通过——designedConclusion 字段真值行合计 10+ 条真实端到端验证 S2 收敛/报告内核成立）；soak 机制升级（跨房间生命周期、spawn 增量聚合、soak facilitator 可配）落地并经五次真实尝试验证，**≥15min 持续负载合同由 claude(cld)-facilitator 变体达成（attempt 5：932372ms、15 轮完成、10 房 9 次 designedConclusion 正确处理、spawn 实测=加法公式、不变量全绿）**。本计划全部阶段交付完成，显式带两个开放项（不宣称「全量通过」）：(a) codex-facilitator soak 变体 ≥15min 证据待 codex app-server 稳定窗口补齐（合同已由 claude 变体达成，此项为补齐性质）、(b) launchd 崩溃拉起人工项待用户执行。