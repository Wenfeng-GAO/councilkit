...
3. `plan`/`format` 仍无独立 `consumeRetry` 调用点。
4. 未跑真实用户 PR / 未 push。
...
| command | exit |
| --- | --- |
| vitest（gate-policy / candidate-verify / checkout-pr / v2-loop / panel / host-mutation） | 0（60 tests） |
| `pnpm typecheck` | 0 |
| `pnpm build:cli` / `pnpm build:host` | 0 |
| `pnpm exec playwright test -c playwright.repair-ui.config.ts` | 0（1 test） |
| UI screenshot | `/tmp/councilkit-cursor-convergence-20260921/ui-smoke.png`（49042 bytes） |
